from flask import Flask, request, jsonify, url_for, make_response, render_template, send_file
...
class Vrt(db.Model):
    ...
    cache = db.Column(db.DateTime, nullable=True, default=datetime.datetime.utcnow)
    ...
    def __init__(self, naziv, wiki, latnaziv, cache):
...
def add_entry():
    try:
        ...
        return make_response(jsonify({'status': "Can not create",
                        ...
                        'response': None}), 400)
    delta = datetime.datetime.utcnow() - timedelta(minutes=1)
    new_entry = Vrt(naziv, wiki, latnaziv, delta)
    ...
    return make_response(jsonify({'status': "OK",
                        ...
                        'response': {
                         ...
                         'links': {'self' : url_for('get_single_entry', sifra=sifra),
                                   ...
                                   'slika' : url_for('get_picture', sifra=sifra)
                             }
                        }), 200)
...
@app.route('/slika/<int:sifra>', methods=['GET'])
def get_picture(sifra):
    vrt = Vrt.query.get(sifra)
    page = wikipedia.page(vrt.wiki)
    link = page.images[0]
    filename = link.split('/')[-1]
    delta = datetime.datetime.utcnow() - timedelta(minutes=1)
    if (vrt.cache < delta) or not os.path.isfile(filename):
        vrt.cache = datetime.datetime.utcnow()
        db.session.commit()
